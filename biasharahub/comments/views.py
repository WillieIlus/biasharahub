...
class CommentPostPoll(LoginRequiredMixin, CreateView):
    model = Comment
    form_class = CommentForm
    template_name = 'includes/form.html'

    def form_valid(self, form):
        form.instance.content_object = get_object_or_404(PostPoll, slug=self.kwargs['slug'])
        form.instance.user = self.request.user
        parent_obj = None
        try:
            parent_id = int(self.request.POST.get("parent_id"))
    ...
    def get_success_url(self):
        # return reverse('reviews:detail', kwargs={'slug': self.object.content_object.slug})
        return reverse('post:detail', kwargs={'slug': self.object.content_object.slug})
