...
@register.simple_tag
def is_open(biashara=None, attr=None):
    """
    Returns False if the biashara is closed, or the OpeningHours object
    to show the biashara is currently open.
    """
    obj = utils.is_open(biashara)
    if obj is False:
        return False
    if attr is not None:
...
@register.simple_tag
def is_open_now(biashara=None, attr=None):
    """
    Returns False if the biashara is closed, or the OpeningHours object
    to show the biashara is currently open.
    Same as `is_open` but passes `now` to `utils.is_open` to bypass `get_now()`. 
    """
    obj = utils.is_open(biashara, now=datetime.datetime.now())
    if obj is False:
        return False
    if attr is not None:
        return getattr(obj, attr)
...
def next_time_open(biashara):
    ...
    if the biashara is currently open or if there is no such object.
    """
    obj, ts = utils.next_time_open(biashara)
    return obj


@register.filter(expects_localtime=True)
def has_closing_rule_for_now(biashara, attr=None):
    obj = utils.has_closing_rule_for_now(biashara)
    if obj is False:
        return False
    if attr is not None:
        return getattr(obj, attr)
...
@register.filter(expects_localtime=True)
def get_closing_rule_for_now(biashara, attr=None):
    obj = utils.get_closing_rule_for_now(biashara)
    if obj is False:
        return False
    if attr is not None:
...
@register.simple_tag
def opening_hours(biashara=None, concise=False):
    """
    ...
    # Without `biashara`, choose the first company.
    if biashara:
        ohrs = OpeningHours.objects.filter(company=biashara)
    else:
        try:
            Biashara = Business
            ohrs = Biashara.objects.first().opening_hours.all()
